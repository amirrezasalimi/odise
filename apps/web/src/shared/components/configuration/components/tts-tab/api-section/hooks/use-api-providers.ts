...
export function useApiProviders() {
    ...
    const configQuery = useQuery(api.apis.config.getConfig, { key: CONFIG_KEYS.api_tts_providers });
    ...
    useEffect(() => {
        if (configQuery) {
            try {
                const config = JSON.parse(configQuery as string) as { providers: ApiTTSProviderItem[] };
